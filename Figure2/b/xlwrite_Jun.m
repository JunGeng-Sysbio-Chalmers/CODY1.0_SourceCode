function [status, message]=xlwrite(filename,A,sheet, range)  %% change 137
...
% load into path 
pathname=regexp(which('xlwrite'),'(?<name>.*)\\.*.m','names'); 
loc=sprintf('%s%cpoi_library%c',pathname.name,filesep,filesep); 

jars=strcat(loc,{'poi-3.8-20120326.jar';'poi-ooxml-3.8-20120326.jar';... 
