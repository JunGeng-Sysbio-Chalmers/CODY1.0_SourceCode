...
# load data
####################
B_file <- "./new_data_formula_mixed/B_formula_mixed_8_ALL.txt"
B_data <- read.table(file=B_file, sep="\t", header=T, stringsAsFactors=F)

M4_file <- "./new_data_formula_mixed/M4_formula_mixed_8_ALL.txt"
M4_data <- read.table(file=M4_file, sep="\t", header=T, stringsAsFactors=F)

M12_file <- "./new_data_formula_mixed/M12_formula_mixed_8_ALL.txt"
...
# read species info
species_file <- "./data_original/Bac8_species.txt"
...
# main5_boxplot <- ggplot(aes(y=RelativeAbundance, x=Type, fill=Species, col=Species), data=min4_data)
...
#main5_plot <- ggplot(main5_data, aes(y=RelativeAbundance, x=Type, fill=Species, shape=Type)) + 
# main5_plot <- ggplot(main5_data, aes(y=RelativeAbundance, x=Type, shape=Type)) + 
#   xlab("") + 
#   ylab("Relative abundance") +
#   #geom_jitter(size=2, position=position_jitter(0.2), alpha=0.6, colour="grey2") +
...
main5_plot <- ggplot(main5_data, aes(y=RelativeAbundance, x=Type, shape=Type)) + 
  ...
  # scale_fill_manual(values=color_12255) +
  # scale_color_manual(values=color_12255) +
...
prediction_file <- "./data_prediction/Prediction.txt"
