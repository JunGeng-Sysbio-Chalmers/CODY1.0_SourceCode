...
baseline=RA_data{:,3};  %%  RA
total_RA_bl=sum(baseline);
ratio=baseline./total_RA_bl;

load('Result_0719.mat');
T=ResulsBL.T;
Y=ResulsBL.Y;
Y_model=ResulsBL.Y_model;
Y_mets=ResulsBL.Y_mets;
...
met_co=met_co_hmo;
Time_bl=T;
Time_str=find((Time_bl-600)>0.01); 
...
ini_bl_enz{9}=[ini_bl_enz{9} e_ini(lenz_bl+1:length(e_ini))']';  
...
store_flag=0;
main_10tank(hmo,mucin,fiber,rs,nspc);
...
function main_10tank(hmo,mucin,fiber,rs,nspc)
...
k_hyd_5spc=k_hyd.*[0.9524 1 1 2 0.5 1.23 0.4]';
