function dy=qssm_comm_adult_CM_baseline(t,y,deltat,para,ns,FS,FV,V_colon,sub_index,hmo_in_y,enz_inx_y)
global  spc 
...
k=10;  
    ...
    V_rtm=V_colon(9); V_BLD=V_colon(10);    %% rectum and blood volume
    rho=0.7*1000;    %% assume fece density lower than water(1 g/cm3), transfer to g/L,
   %% caculate each total transport mass from lumen phase to mucosa phase%%
    JLT={};
    JL_temp=0;
    Mass_temp=0;   
    for k=1:4
 ...
 tc=24;   %% tc is the critical time that V4 is full and needed to be empty
 T=5/60;  %% T is the time during which V4 is emptied. here 5min, 5min/60=1/12h     
       if F5==0
...
k=4;    %% VL4, different with VL1-VL3, no f_back(k+1) term
   icr=(k-1).*ylg;
     icr_pre=(k-2).*ylg;
     icr_next=(k).*ylg;  
     icr_m=(k-1+4)*ylg;
