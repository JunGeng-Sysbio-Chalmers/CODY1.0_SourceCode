...
function  concate_BSL_Intv(Result_BF,Result_SF,out_filename)
% read 4m results
...
Tn=length(Time_4m);
New_Time_4m=Time_4m(1:SmpPnt:Tn);

T=[Time_4m(1:Tn-1,:);Time_12m+612];
Y=[Y_4m(1:Tn-1,:);Y_12m];
...
% bls_index=find(T==Time_4m(length(Time_4m)-1))
bls_index=find(T==New_Time_4m(length(New_Time_4m)-1));

% AC_baseline=Ymodel_mass_rctm(length(New_Time_4m),1:nspc);
...
k=10;
Ymodel{k}=Y(:,(k-1).*ylg+1+lxini+1:(k-1).*ylg+1+lxini+lxini-nspc);    %% no bacteria, only metabolites

colnames_bac=strrep(met_udf_co(1:nspc),'BIOMASS_','');  % remove prefix of Biomass_, as well as the blank
...
%% the one that is only used to comparison by users
...
%% the one that are used as input of R shiny server to draw adult plot, bot plot and dot plot
output=[colnames_bac' ;num2cell(RA_4m_pre);num2cell(RA_12m_pre)];
rowname={'Time'  'Baseline RA' 'Intervention RA' }';
output=[rowname output];
% writetable(cell2table(output),'prediction_format.csv','WriteVariableNames',false); 
