function dy=qssm_comm_adult_CM_Intervention(t,y,deltat,para,ns,FS,FV,V_colon,sub_index,hmo_in_y,enz_inx_y)
global  spc 
...
HMO_inx=hmo_in_y;
        ...
        if (mod(t,24)-1-0.01)>=0&&(mod(t,24)-2-0.01)<=0
            HMO_in=HMO_const;
        elseif (mod(t,24)-7-0.01)>=0&&(mod(t,24)-8-0.01)<=0
            HMO_in=HMO_const;
        elseif (mod(t,24)-14-0.01)>=0&&(mod(t,24)-15-0.01)<=0
            HMO_in=HMO_const;
        else
            HMO_in=0;
        end
...
k=4;    
 ...
 icr_bld=9*ylg+1+lx;
  ...
  for k=6:8   %% VM2-VM4
    ...
    icr_bld=9*ylg+1+lx;
     detachment(:,k)=kd(k-4).*y(1+icr:ns+icr)./(C_Kd(k-4)+y(1+icr:ns+icr)).*y(1+icr:ns+icr);
     dy(1+icr:ns+icr)=dy(1+icr:ns+icr)-detachment(:,k);   %% X bacteria in mucosa, only growth and detachment
