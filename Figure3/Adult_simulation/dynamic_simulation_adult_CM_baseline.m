function [Tmodel,Y,Ymodel,Ymodel_met,met_udf_co]=dynamic_simulation_adult_CM_baseline(community,n_spc,num_rct,FSG,Feed,flow_back,k,V,ini_stat,time,length_each_tank,km_lumen,Pn_mucosa,km_water,density,Met_Mass,hmo_para,round,vld,uv)
...
%      ke{s}=spc{s}.ke;
     ...
     n_carbon{s}=spc{s}.n_carbon;  %% n_carbon should be in column format
     [cr,cl]=size(n_carbon{s}); %% to see how many substrates of this species use
     if (cr>1)&&(cl>1)
...
[T Y]=my_runge_kutta4(@qssm_comm_adult_CM_baseline,y0,tspan,kmax,n_s,FSG,Feed,V,substrate_index,HMO_index,enzyme_index); 
