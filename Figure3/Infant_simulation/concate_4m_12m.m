%% concate the 4m result and 12m result, construct the whole profiles
...
% initiate the 4month data, same column number as the 12 month
...
Aim_4m=RA_data.x4M(1:nspc_12m)'.*100;%
Aim_12m=RA_data.x12M(1:nspc_12m)'.*100;%
...
RA_4m_pre=AC_4m./sum(AC_4m).*sum(Aim_4m);   
RA_12m_pre=AC_12m./sum(AC_12m).*sum(Aim_12m);
k=10;
...
lg=ylg_12m;
% write_tank_table(Concate_0to12m_Y,Ymodel,Ymodel_met,met_co_12m,lg,lxini_12m,rct,nspc_12m,filename);
