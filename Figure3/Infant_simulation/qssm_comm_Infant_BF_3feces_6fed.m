function dy=qssm_comm_Infant_BF_3feces_6fed(t,y,deltat,para,ns,FS,FV,V_colon,sub_index,hmo_in_y,enz_inx_y)
global  spc 
...
HMO_inx=hmo_in_y;
         V5=y(ylg*9+1);  
        if (mod(t,24)-3-0.01)>=0&&(mod(t,24)-3.5-0.01)<=0
            HMO_in=HMO_const;
        elseif (mod(t,24)-6.5-0.01)>=0&&(mod(t,24)-7-0.01)<=0
            HMO_in=HMO_const;
        elseif (mod(t,24)-10-0.01)>=0&&(mod(t,24)-10.5-0.01)<=0
            HMO_in=HMO_const;
        elseif (mod(t,24)-13.5-0.01)>=0&&(mod(t,24)-14-0.01)<=0
            HMO_in=HMO_const;
        elseif (mod(t,24)-17-0.01)>=0&&(mod(t,24)-17.5-0.01)<=0
            HMO_in=HMO_const;
        elseif (mod(t,24)-20.5-0.01)>=0&&(mod(t,24)-21-0.01)<=0
            HMO_in=HMO_const;
        else
            HMO_in=0;
        end
...
%      D=FV(k)/V_colon(k);
     ...
     icr_m=(k-1+4)*ylg;
     detachment(:,k)=kd(k).*y(1+icr_m:ns+icr_m)./(C_Kd(k)+y(1+icr_m:ns+icr_m)).*y(1+icr_m:ns+icr_m).*V_colon(k+4)./V_colon(k);
     dy(1+icr:ns+icr)=dy(1+icr:ns+icr)+FL(k)/V_colon(k).*y(1+icr_pre:ns+icr_pre)-FL(k+1)/V_colon(k)*y(1+icr:ns+icr)-f_back(k)/V_colon(k).*y(1+icr:ns+icr)+f_back(k+1)/V_colon(k).*y(1+icr_next:ns+icr_next)+detachment(:,k);        %%
...
k=4;    %%
   icr=(k-1).*ylg;
     ...
     dy(1+icr:ns+icr)=dy(1+icr:ns+icr)+FL(k)/V_colon(k).*y(1+icr_pre:ns+icr_pre)-FL(k+1)/V_colon(k)*y(1+icr:ns+icr)-f_back(k)/V_colon(k).*y(1+icr:ns+icr)+detachment(:,k);        %% biomass of each species in the fluid  
...
%      dy(1+icr:ns+icr)=dy(1+icr:ns+icr)-detachment(:,k-4).*V_colon(k-4)./V_colon(k);   %%
     ...
     dy(hmo_ix+icr)=dy(hmo_ix+icr)+FM(k-4)/V_colon(k).*HMO_in-FM(k-4+1)/V_colon(k)*y(hmo_ix+icr)+V_colon(k-4)/V_colon(k).*km(1)*(y(hmo_ix+icr_lm)-y(hmo_ix+icr))-Pn(1)*(y(hmo_ix+icr)-y(hmo_ix-ns+icr_bld))-r_hyd_hmo{k};   %%
     dy(ind_hex+icr)=dy(ind_hex+icr)-FM(k-4+1)/V_colon(k)*y(ind_hex+icr)+V_colon(k-4)/V_colon(k).*km(2)*(y(ind_hex+icr_lm)-y(ind_hex+icr))-Pn(2)*(y(ind_hex+icr)-y(ind_hex-ns+icr_bld))+r_hyd_hex{k};   %%
...
%     dy(1+icr:ns+icr)=dy(1+icr:ns+icr)-detachment(:,k-4).*V_colon(k-4)./V_colon(k);   %% 
     detachment(:,k)=kd(k-4).*y(1+icr:ns+icr)./(C_Kd(k-4)+y(1+icr:ns+icr)).*y(1+icr:ns+icr);
     dy(1+icr:ns+icr)=dy(1+icr:ns+icr)-detachment(:,k);   %%
    dy(hmo_ix+icr)=dy(hmo_ix+icr)+FM(k-4)/V_colon(k).*y(hmo_ix+icr_pre)-FM(k-4+1)/V_colon(k)*y(hmo_ix+icr)+V_colon(k-4)/V_colon(k).*km(1)*(y(hmo_ix+icr_lm)-y(hmo_ix+icr))-Pn(1)*(y(hmo_ix+icr)-y(hmo_ix-ns+icr_bld))-r_hyd_hmo{k};   %%

    dy(ind_hex+icr)=dy(ind_hex+icr)+FM(k-4)/V_colon(k).*y(ind_hex+icr_pre)-FM(k-4+1)/V_colon(k)*y(ind_hex+icr)+V_colon(k-4)/V_colon(k).*km(2)*(y(ind_hex+icr_lm)-y(ind_hex+icr))-Pn(2)*(y(ind_hex+icr)-y(1+icr_bld))+r_hyd_hex{k};   %%
 ...
 k=9;    %%
     ...
     icr_next=(k).*ylg;
               ...
               dy(1+icr:ns+icr)=dy(1+icr:ns+icr)-FM(k-4)/V5*y(1+icr:ns+icr)+FL(k-4)/V5*(y(1+icr_lm:ns+icr_lm)-y(1+icr:ns+icr));        %% d(x1-x5)/dt
               ...
               dy(icr+ylg+1+ns+1:icr+ylg+1+lx)=V5*dSP_dt_V5_temp+FL(k-4)*y(ns+1+icr_lm:lx+icr_lm)+FM(k-4)*y(ns+1+icr_pre:lx+icr_pre)-F5*y(ns+1+icr:lx+icr);        %% d(HexoseV5-H2V5)/dt
