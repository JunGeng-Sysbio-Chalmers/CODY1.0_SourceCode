...
prd_file <- "./data/prediction_format.csv"     ##
...
colnames(prd_data)<-str_replace(colnames(prd_data),"Bacteroides_","B.")
colnames(prd_data)<-str_replace(colnames(prd_data),"Bifido_","B.")
colnames(prd_data)<-str_replace(colnames(prd_data),"Eubacterium_","E.")
colnames(prd_data)<-str_replace(colnames(prd_data),"Faecalibacterium_prausnitzii","F.pr")
colnames(prd_data)<-str_replace(colnames(prd_data),"Roseburia_","R.")

prd_data$type <- rownames(exp_data)
prd_data<-prd_data[,-1]
...
prd_data_list <- prd_data %>% gather("Species", "Prediction", -type)


all_data <- full_join(exp_data_list, prd_data_list, by=c("type", "Species"))
head(all_data)
...
baseline_data <- all_data %>% filter(type == "Baseline")
baseline_data$Metagenomics<-baseline_data$Metagenomics/100
baseline_data$Prediction<-baseline_data$Prediction
...
intervention_data$Prediction<-intervention_data$Prediction
...
intervention_plot
# all_plot1<-grid.arrange(baseline_plot, intervention_plot)
all_plot <- ggarrange(baseline_plot, intervention_plot, ncol=2,
                      widths = c(1.1, 1.1), align="v")
# all_plot

ggsave("./fig/baseline_intervention_plot_v4_0802.svg", all_plot,height = 4.6, width = 10.3, units = "cm")
