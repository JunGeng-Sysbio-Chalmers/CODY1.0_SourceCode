...
main5_plot <- ggplot(main5_data, aes(y=RA, x=Time, shape=Time)) + 
  ...
  scale_x_discrete(labels=c("0" = "D0", "14" = "D14")) +
...
prediction_file <- "./data_prediction/prediction_format.csv"
...
other3_predict_plot <- ggplot(other3_prediction, aes(y=prediction, x=Time, fill=strain, shape=Time)) + 
  ...
  # ) +
  facet_grid(.~strain) +
  scale_x_discrete(labels=c("0" = "D0", "14" = "D14", "12 Months" = "12")) +
...
ggsave(plot=multi_plot, filename="multi_plot_0802.pdf", width=18, height=11, units="cm")
