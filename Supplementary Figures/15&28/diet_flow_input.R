...
## nutrients input ##
# hours<-c(0,2.5,3,6,6.5,9.5,10,13,13.5,16.5,17,20,20.5,24)
...
nutrients_4month_6diets<-stepfun(hours,c(0,0,50,0,50,0,50,0,50,0,50,0,50,0,0),right=FALSE)
pdf("nutrients_0-4month_6diets_3.pdf",width=8.92,height=4.1)
...
hours<-c(0,7,8,12,13,17,18,24)
nutrients_12m<-fortify(stepfun(hours,c(0,0,100,0,100,0,100,0,0),right=FALSE))
pdf("nutrients_4-12month-0913.pdf",width=8.92,height=4.1)
...
plot(nutrients_12m,do.points=FALSE,
     # tcl=0.4, tck=0.03,
     xlim = c(0,24), ylim = c(-0.2, 150),yaxs="i",
     axes=FALSE,
     # col="#cc4c02",
     col="#961563",lwd = 1.8,
     cex.lab = 1.15, 
...
axis(side=1, at=seq(0,24,by=6), tcl=-0.4, tck=-0.03,
     ...
     col.lab="black", 
     font.lab=2, font.axis=2)
axis(side = 2,at=seq(0,120,by=60), tcl=-0.3, tck=0.03,
     cex.lab = 1.15, 
     cex.axis=0.9,
     col.lab="black", 
     font.lab=2, font.axis=2)
...
## 4m data plot
data_4m<-read.table('4m_24h_data_comparison_20180911.txt',header=T, row.names=1, sep="\t", stringsAsFactors=T)
data_4m<-data_4m[,-3]
...
plot_4m
ggsave(plot = plot_4m, filename = "./fig/data_4m_24h.pdf", width = 8, height = 3)

## 12m data plot
# nutrients_12m$
data_12m<-read.table('12m_24h_data_comparison_20180911.txt',header=T, row.names=1, sep="\t", stringsAsFactors=T)
# data_4m<-data_4m[,-3]
time<-rownames(data_12m)
time<-str_replace(time,',','.')
time<-as.numeric(time)
index_Str<- which(time==137)
index_end<- which(time==161)
# index_Str<- which(time==127)
# index_end<- which(time==151)
data_12m_24h<-data_12m[index_Str:index_end,]
...
plot_12m <- ggplot(data_12m_24h_tt,aes(time, value, colour=variable)) +
  geom_line(linetype = "solid", size=0.75) +
  ...
  # scale_x_continuous(limits = c(140, 164), expand = c(0, 0)) +
    ...
    scale_y_continuous(limits = c(-0.2, 150), expand = c(0, 0)) +
  # scale_fill_manual(values=c("#238b45", "#4eb3d3", "#e31a1c", "#fdbf6f")) +  ## previous color
  # scale_color_manual(values=c("#f6e8c3","#dfc27d", "#bf812d","#8c510a","#c7eae5","#80cdc1","#35978f","#01665e","#e0a494","#90933d")) +    ## add circle line
  # scale_color_brewer(3,type = "qual")+
...
plot_12m
ggsave(plot = plot_12m, filename = "./fig/data_12m_24h.pdf", width = 8, height = 3)
