...
k=10;  
    ...
    for k=1:4
        for i=1:lx
            ...
            JL_temp=JL_temp+km(i)*mol_mass(i).*(y(i+(k-1)*ylg)-y(i+(k-1+4)*ylg));   %% add water absorption,g/L h
        end
        JLT{k}=JL_temp; %% g/L h
    ...
    JMT={};
    JM_temp=[0];
    Mass_mucosa_temp=0;   %% water content
    icr_blood=9*ylg+1+lx;  %% end index of rectum
    for k=5:8
        for i=1:lx
 ...
 T=2/60;  %% T is the time during which V4 is emptied. here 5min, 5min/60=1/12h     
...
k=4;    %% 
 ...
 dy(hmo_ix+icr)=dy(hmo_ix+icr)+FM(k-4)/V_colon(k).*HMO_in-FM(k-4+1)/V_colon(k)*y(hmo_ix+icr)+V_colon(k-4)/V_colon(k).*km(1)*(y(hmo_ix+icr_lm)-y(hmo_ix+icr))-Pn(1)*(y(hmo_ix+icr)-y(hmo_ix+icr_bld));   %%??????????
 ...
 for k=6:8   %% VM2-VM4
    ...
    dy(hmo_ix+icr)=dy(hmo_ix+icr)+FM(k-4)/V_colon(k).*y(hmo_ix+icr_pre)-FM(k-4+1)/V_colon(k)*y(hmo_ix+icr)+V_colon(k-4)/V_colon(k).*km(1)*(y(hmo_ix+icr_lm)-y(hmo_ix+icr))-Pn(1)*(y(hmo_ix+icr)-y(hmo_ix+icr_bld));   %%???????
...
k=10;   %% caculate blood model mass balance, no reaction take place here, only mass transport
icr=(k-1).*ylg;
icr_M1=(k-4-1-1).*ylg;
icr_M2=(k-4-1-1+1).*ylg;
icr_M3=(k-4-1-1+2).*ylg;
icr_M4=(k-4-1-1+3).*ylg;
lx_bld=icr+1+lx;   %% length of state variables for previous 9 tanks
CB0=0.5.*ones(lx,1);   %%
CB0(1)=0.1;   %%?
