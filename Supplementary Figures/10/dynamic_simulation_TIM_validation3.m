...
out_time=[];
t_open=[];V5_open=[];
[T Y]=my_runge_kutta4_TIM(@qssm_comm_TIM_validation3,y0,tspan,FSG,Feed,V);  
...
% plot = figure('Name','Three reactor biomass and metabolites Plot');
pred_set='Validation2';
Vad1_lumen=figure(1);
for k=1:4
...
%         title(['One time Pulse Tracor'],'fontsize',16,'fontname','Helvetica');
    ...
    xlabel('Time [h]','fontsize',21,'fontname','Helvetica'); ylabel(['Hexose ' '[mM]'],'fontsize',21,'fontname','Helvetica');
...
% saveas(gcf, 'test.png');
export_fig Validation3_lumen_mets.pdf -q101
...
%         title(['Mucus Response'],'fontsize',16,'fontname','Helvetica');
        bms=legend(Mucus_legend,'linewidth',1.5,'fontsize',21,'fontname','Helvetica');
        set(bms,'Box','off','Location','NorthEast')
...
export_fig Validation3_mucus_mets.pdf -q101
...
%     xlabel('Time [h]','fontsize',16,'fontname','helvetica'); 
    ylabel(['Mets [mol/L]'],'fontsize',21,'fontname','helvetica');
    ylim([0,0.05])
%     subplot(3,2,4)   %% ??????9??????????????rectum?????????????????????????????????????????????????????????????
    subplot(2,1,2)   %% ??????9??????????????rectum?????????????????????????????????????????????????????????????

    plot(Tmodel,V5_rctm,'k-','linewidth',2);
%     title('Feces Volume [L]','fontsize',10,'fontname','Helvetica'); 
        set(gca,'YTick',[0 0.1]);
    set(findobj(get(gca,'Children'),'LineWidth',1.5),'LineWidth',2);
        set(gca,'fontsize',21); set(gca,'linewidth',1.5);
...
export_fig Validation3_feces_mets.pdf -q101

    
k=10;
V_BLD=figure(4)    %% blood??????????????????????????????????????
...
%        title('Blood Metabolites','fontsize',16,'fontname','Helvetica');
%        mts=legend(met_udf_co(1:lxini),'linewidth',1.5,'fontsize',16,'fontname','helvetica');
%        set(mts,'Box','off','Location','NorthEast')
    set(gca,'fontsize',21); set(gca,'linewidth',1.5);
    set(findobj(get(gca,'Children'),'LineWidth',1.5),'LineWidth',2);
    xlabel('Time [h]','fontsize',21,'fontname','Helvetica'); ylabel(['Blood Metabolites' '[mM]'],'fontsize',21,'fontname','Helvetica');
    hold off
% export_fig 'Validation1_Blood_mets.tiff'
export_fig Validation3_Blood_mets.pdf -q101


V_BLD=figure(5)    %% blood??????????????????????????????????????
...
export_fig Validation3_Inflow_Outflow.pdf -q101
...
for i=1:length(Tmodel)
      ...
      if (mod(Tmodel(i),72)-0-0.01)>=0&&(mod(Tmodel(i),72)-0.1-0.01)<=0
            a=FSG;
       elseif (mod(Tmodel(i),72)-10-0.01)>=0&&(mod(Tmodel(i),72)-10.1-0.01)<=0
            a=FSG;
        elseif (mod(Tmodel(i),72)-20-0.01)>=0&&(mod(Tmodel(i),72)-20.1-0.01)<=0
            a=FSG;
        else
            a=0;
      end
   ...
   plot(Tmodel,HMO_feeding./1000*180,'color',color{2},'linewidth',2);
...
export_fig Validation3_Feeding_hexose.pdf -q101
