...
out_time=[];
t_open=[];V5_open=[];
[T Y]=my_runge_kutta4_TIM(@qssm_comm_TIM_validation5,y0,tspan,FSG,Feed,V);  
...
% saveas(gcf, 'test.png');
export_fig Validation5_lumen_mets.pdf -q101
...
%         title(['Feces Metabolites'],'fontsize',20,'fontname','Helvetica');
%         bms=legend(met_udf_co(1:n_s),'linewidth',1.5,'fontsize',7);
%         set(bms,'Box','off')
...
export_fig Validation5_feces_mets.pdf -q101
...
export_fig Validation5_Blood_mets.pdf -q101


V_BLD=figure(5)    
    
    inflow=0.004*ones(length(Tmodel),1);
        plot(Tmodel,inflow,'color',color{6},'linewidth',3);

    hold on
        plot(out_time,outflow,'color',color{5},'linewidth',2);

    hold off
    mts=legend({'Feeding rate' 'Outflow rate'},'linewidth',1.5,'fontsize',21,'fontname','Helvetica');
       set(mts,'Box','off','Location','NorthEast')
    ylim([0,0.006]);
        set(gca,'fontsize',21); set(gca,'linewidth',1.5);
    set(findobj(get(gca,'Children'),'LineWidth',1.5),'LineWidth',2);
...
export_fig Validation5_Inflow_Outflow.pdf -q101
...
for i=1:length(Tmodel)
      ...
      if (mod(Tmodel(i),72)-0-0.01)>=0&&(mod(Tmodel(i),72)-2-0.01)<=0
            a=FSG;
...
export_fig Validation5_Feeding_hexose.pdf -q101
