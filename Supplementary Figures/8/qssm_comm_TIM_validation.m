...
lx=length(met_udf_co);
        ...
        if (mod(t,72)-0-0.01)>=0&&(mod(t,72)-0.1-0.01)<=0
            HMO_in=HMO_const;
%         elseif (mod(t,72)-6-0.01)>=0&&(mod(t,72)-6.1-0.01)<=0
%             HMO_in=HMO_const;
...
k=1;     %% VL1
     ...
     dy(hmo_ix+icr)=dy(hmo_ix+icr)+FL0/V_colon(k).*HMO_in-FL(k+1)/V_colon(k)*y(hmo_ix+icr)+f_back(k+1)/V_colon(k).*y(hmo_ix+icr_next)-km(1)*(y(hmo_ix+icr)-y(hmo_ix+icr_m));   %%????????????????????????????hmo??????????????????????????????hmo??????????????????g/L??????????????????????????????????????????????????????

%% mass balance equation of the second and third lumen reactors; VL2-VL3 %%
   for k=2:3
     icr=(k-1).*ylg;
     icr_pre=(k-2).*ylg;
     icr_next=(k).*ylg;  
...
CB0=0.5.*ones(lx,1);   %% assume inlow concentration of blood is very low
CB0(1)=0.1;   
