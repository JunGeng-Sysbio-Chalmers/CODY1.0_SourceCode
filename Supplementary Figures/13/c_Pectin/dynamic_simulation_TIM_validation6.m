...
for k=1:8
    ...
    hAxis(k) =subplot(4,2,2*k-1)
       for i=1:lxini
           ...
           ylabel('Conc. [mmol/L]','fontsize',16,'fontname','Helvetica');
           title(title_name{k},'fontsize',18,'fontname','Helvetica','FontWeight','normal');
           hold on
       end
       hold off
...
%        title('Blood SCFA','fontsize',21,'fontname','Helvetica');
...
%             a=1000;
   ...
   plot(Tmodel,HMO_feeding,'color',tank{2}./255,'linewidth',2);
    ...
    mts=legend({'Feeding Pectin'},'linewidth',1.5,'fontsize',19,'fontname','Helvetica');
       set(mts,'Box','off','Location','NorthEast')
    ylim([0,HMO_g*2]);
...
%    plot(Tmodel,SCFA_tt(:,i),'color',tank{i+1}./255,'linewidth',2);
    ...
    title('SCFA production by Pectin', 'fontname','Helvetica','FontWeight','normal','FontSize',24);
...
writetable(output,'Pectin_SCFA.csv');    
