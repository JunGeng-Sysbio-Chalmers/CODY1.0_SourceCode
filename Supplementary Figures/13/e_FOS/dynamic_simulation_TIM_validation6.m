...
In_and_out_flow=figure(4)
    ...
    hold off
    mts=legend({'Feeding rate' 'Removal rate'},'linewidth',1.5,'fontsize',19,'fontname','Helvetica');
       set(mts,'Box','off','Location','NorthEast')
    ylim([0,0.006]);
        set(gca,'fontsize',16); set(gca,'linewidth',1.5);
    set(findobj(get(gca,'Children'),'LineWidth',1.5),'LineWidth',2);
...
%             a=1000;
   ...
   plot(Tmodel,HMO_feeding,'color',tank{2}./255,'linewidth',2);
    ...
    mts=legend({'Feeding FOS'},'linewidth',1.5,'fontsize',19,'fontname','Helvetica');
       set(mts,'Box','off','Location','NorthEast')
    ylim([0,HMO_g*2]);
...
writetable(output,'FOS_SCFA.csv');
