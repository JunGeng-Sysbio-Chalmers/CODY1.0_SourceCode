...
k=10;  
   ...
   %% caculate each total transport mass from mucosa phase to blood phase%%
    ...
    for k=5:8
        for i=1:lx
            Mass_mucosa_temp=Mass_mucosa_temp+mol_mass(i).*(y(i+(k-1)*ylg));    %% ??????????????????g/mmol*mmol/L=g/L(?????????????????????????????????????????????????????????????)
...
k=1;     
     ...
     icr_next=(k).*ylg; 
     icr_m=(k-1+4)*ylg;
    dy(hmo_ix+icr)=dy(hmo_ix+icr)+FL0/V_colon(k).*HMO_in-FL(k+1)/V_colon(k)*y(hmo_ix+icr)+f_back(k+1)/V_colon(k).*y(hmo_ix+icr_next)-km(hmo_ix)*(y(hmo_ix+icr)-y(hmo_ix+icr_m))-r_hyd_hmo{k};  
     dy(hmo_ix+1+icr:lx+icr)=dy(hmo_ix+1+icr:lx+icr)-FL(k+1)/V_colon(k)*y(hmo_ix+1+icr:lx+icr)+f_back(k+1)/V_colon(k).*y(hmo_ix+1+icr_next:lx+icr_next)-km(2:end).*(y(hmo_ix+1+icr:lx+icr)-y(hmo_ix+1+icr_m:lx+icr_m))+r_hyd_hex{k};  
