...
# ggexport(phylum_plot, filename="8_species_phylum_plot_formula_mixed.jpg", width=250, height=900, res=150, pointsize = 10)
# ggsave(plot=phylum_plot, filename="8_species_phylum_plot_formula_mixed.pdf", width=4, height=12, units="cm")


# get the information for each group
group_mean_abundance <- phylum_abundance_data %>% group_by(time, phylum) %>% summarise(mean(value))
write.table(group_mean_abundance, file="8_species_phylum_abundance_mean__formula_mixed.txt", sep="\t", quote=F, row.names = F)

group_median_abundance <- phylum_abundance_data %>% group_by(time, phylum) %>% summarise(median(value))
write.table(group_median_abundance, file="8_species_phylum_abundance_median_formula_mixed.txt", sep="\t", quote=F, row.names = F)
